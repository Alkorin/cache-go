...
func Test(t *testing.T) {
	...
	// Concurrent get
	wg.Add(4)
	go func() { foo1 = cache.Get("foo","Cached-foo").(string); wg.Done() }()
	go func() { bar1 = cache.Get("bar","Cached-bar").(string); wg.Done() }()
	go func() { foo2 = cache.Get("foo","Cached-foo").(string); wg.Done() }()
	go func() { bar2 = cache.Get("bar","Cached-bar").(string); wg.Done() }()
	wg.Wait()

	// Sequential get from cache
	wg.Add(2)
	go func() { foo3 = cache.Get("foo","Cached-foo").(string); wg.Done() }()
	go func() { bar3 = cache.Get("bar","Cached-bar").(string); wg.Done() }()
	wg.Wait()

	// Wait cache to expire and retry
	time.Sleep(300 * time.Millisecond)
	wg.Add(1)
	go func() { foo4 = cache.Get("foo","Cached-foo").(string); wg.Done() }()
	wg.Wait()

	// Wait cleanup and retry
	time.Sleep(1 * time.Second)
	wg.Add(1)
	go func() { bar4 = cache.Get("bar","Cached-bar").(string); wg.Done() }()
	wg.Wait()
